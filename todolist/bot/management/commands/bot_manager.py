...
from bot.management.commands.dialogs import (BAD_CATEGORY, BAD_COMMAND,
                                             CODE_DESCRIPTION, CREATE_2,
                                             CREATE_BRAKE, CREATE_SUCCESS,
                                             GREETINGS_LIST, greetings)
from bot.models import TgUser
from bot.tg.client import TgClient
from bot.tg.dc import UpdateObj
from bot.views import (create_goal_from_tg, get_goal_categories,
                       get_goal_category, goals_list_4_tg)
...
class BotManager():
    ...
    def print_categories(self, tg_user: TgUser):
        categories = get_goal_categories(tg_user)
        # TODO: ВЫВОД КНОПОК В ТГ-БОТЕ + кнопка ОТМЕНЫ
        keyboard = self.tg_client.create_keyboard(categories, "ОТМЕНИТЬ")
